...
class DiaryModal(discord.ui.Modal, title="Create a diary page"): 
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cursor.execute("INSERT INTO diary VALUES (?, ?, ?, ?)", (interaction.user.id, self.text.value, self.titl.value, date))
        connection.commit()
...
class roleplay(commands.Cog):
    ...
    async def joint_toggle(self, ctx: commands.Context):
        check = self.bot.db.execute("SELECT * FROM joint WHERE guild_id = ?", (ctx.guild.id,)).fetchone()
        if not check:
            self.bot.db.execute("INSERT INTO joint VALUES (?,?,?)", (ctx.guild.id, 0, ctx.author.id))
            await self.joint_send(ctx, "The joint is yours")
        else:
            self.bot.db.execute("DELETE FROM joint WHERE guild_id = ?", (ctx.guild.id,))
            await ctx.reply(embed=discord.Embed(color=self.bot.color, description=f"{self.smoke} {ctx.author.mention}: Got rid of the server's joint"))
    ...
    async def joint_hit(self, ctx: commands.Context):
        ...
        conn = sqlite3.connect('joint.db')
        c = conn.cursor()
        c.execute("SELECT hits FROM joint WHERE guild_id = ?", (ctx.guild.id,))
        current_hits = c.fetchone()[0]      
        new_hits = current_hits + 1
        embed = discord.Embed(color=self.bot.color, description=f"{self.smoke} {ctx.author.mention}: You just hit the **joint**. This server has a total of **{new_hits}** hits!")
        await mes.edit(embed=embed)
        c.execute("UPDATE joint SET hits = ? WHERE guild_id = ?", (new_hits, ctx.guild.id))
        conn.commit()
        conn.close()
    ...
    async def joint_pass(self, ctx: commands.Context, *, member: discord.Member):
     ...
     self.bot.db.execute("UPDATE joint SET holder = $1 WHERE guild_id = $2", member.id, ctx.guild.id)
    ...
    async def joint_steal(self, ctx: commands.Context): 
     check = self.bot.db.execute("SELECT * FROM joint WHERE guild_id = $1", ctx.guild.id)
     ...
     self.bot.db.execute("UPDATE joint SET holder = $1 WHERE guild_id = $2", ctx.author.id, ctx.guild.id)
    ...
    async def marry(self, ctx: commands.Context, *, member: discord.Member):
     if member == ctx.author: return await ctx.send_error("You can't **marry** yourself")
        ...
        meri = self.bot.db.execute("SELECT * FROM marry WHERE author = $1", member.id)
        if meri is not None: return await ctx.send_warning(f"**{member}** is already married")
        elif meri is None:
           mer = self.bot.db.execute("SELECT * FROM marry WHERE soulmate = $1", member.id)
           if mer is not None: return await ctx.send_warning(f"**{member}** is already married")
                    
        check = self.bot.db.execute("SELECT * FROM marry WHERE author = $1", ctx.author.id) 
        if check is not None: return await ctx.send_warning("You are already **married**. Are you trying to cheat?? 🤨")
        elif check is None:
           check2 = self.bot.db.execute("SELECT * FROM marry WHERE soulmate = $1", ctx.author.id) 
    ...
    async def marriage(self, ctx: commands.Context, *, member: discord.User=None):
      ...
      check = self.bot.db.execute("SELECT * FROM marry WHERE author = $1", member.id)       
      if check is None:
           check2 = self.bot.db.execute("SELECT * FROM marry WHERE soulmate = $1", member.id)
    ...
    async def divorce(self, ctx: commands.Context):       
        ...
        async def button1_callback(interaction):
         ...
         if check is None:
            if check2 is not None: self.bot.db.execute("DELETE FROM marry WHERE soulmate = $1", ctx.author.id)
         elif check is not None: self.bot.db.execute("DELETE FROM marry WHERE author = $1", ctx.author.id)                    
    ...
    async def diary_create(self, ctx: commands.Context): 
      now = datetime.datetime.now()
      date = f"{now.month}/{now.day}/{str(now.year)[2:]}"   
      check = await ctx.bot.db.execute("SELECT * FROM diary WHERE user_id = $1 AND date = $2", ctx.author.id, date)
    ...
    async def diary_delete(self, ctx: commands.Context): 
     ...
     async def select_callback(interaction: discord.Interaction): 
      if interaction.user.id != ctx.author.id: return await interaction.client.ext.send_warning(interaction, "You are not the author of this embed") 
      self.bot.db.execute("DELETE FROM diary WHERE user_id = $1 AND date = $2", ctx.author.id, select.values[0])
