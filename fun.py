...
class RockPaperScissors(discord.ui.View): 
  ...
  async def action(self, interaction: discord.Interaction, selection: str): 
    if interaction.user.id != self.ctx.author.id: return await interaction.client.ext.send(interaction, "This is not your game", ephemeral=True)
...
class fun(commands.Cog):
    ...
    async def esex(self, ctx: commands.Context): 
      ...
      if ctx.voice_client: return await ctx.send("The bot is **already** in a voice channel")
    ...
    @commands.hybrid_command(description="black rate an user", help="fun", usage="<member>")
    async def howblack(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=self.bot.color, title="black r8", description=f"{member.mention} is `{randrange(100)}%` black <:black_hmph:1104383905999634515>")) 

    @commands.hybrid_command(description="racist rate an user", help="fun", usage="<member>")
    async def howracist(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=self.bot.color, title="racist r8", description=f"{member.mention} is `{randrange(100)}%` racist :older_adult:"))      
