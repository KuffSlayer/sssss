...
def is_there_a_reminder(): 
  async def predicate(ctx: Context):
    check = await ctx.bot.db.execute("SELECT * FROM reminder WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, ctx.author.id)
    if not check: await ctx.send("You don't have a reminder set in this server")
...
class Timezone(object):
  ...
  async def get_user_lat_long(self, member: discord.Member): 
   check = self.bot.db.execute("SELECT * FROM timezone WHERE user_id = $1", member.id) 
  ...
  async def tz_set_cmd(self, ctx: Context, location: str):
   ...
   if location is None: return await ctx.send( "Couldn't find a **timezone** for that location")
   check = self.bot.db.execute("SELECT * FROM timezone WHERE user_id = $1", ctx.author.id) 
   obj = TimezoneFinder()
   result = obj.timezone_at(lng=location.longitude, lat=location.latitude)
   if not check: self.bot.db.execute("INSERT INTO timezone VALUES ($1,$2)", ctx.author.id, result)
   else: 
     self.bot.db.execute("DELETE FROM timezone WHERE user_id = $1", ctx.author.id)
     self.bot.db.execute("INSERT INTO timezone VALUES ($1,$2)", ctx.author.id, result)
  ...
  async def get_user_timezone(self, ctx: Context, member: discord.Member): 
   if await self.timezone_request(member) is None: 
    if member.id == ctx.author.id: return await ctx.send( f"You don't have a **timezone** set. Use `{ctx.clean_prefix}tz set` command instead")   
    else: return await ctx.send( f"**{member.name}** doesn't have their **timezone** set")
...
class Utility(Cog):
    def __init__(self, bot: Bot):
        ...
        self.bot.db = sqlite3.connect('snipe.db')
        self.conn = sqlite3.connect("snipe.db")
    ...
    async def clearnames(self, ctx):
        ...
        async def button1_callback(interaction: discord.Interaction): 
          ...
          self.bot.db.execute("DELETE FROM oldusernames WHERE user_id = $1", ctx.author.id) 
    ...
    async def tiktok(self,ctx,username:str):
        try:
          ...
          await ctx.send( "TikTok account **couldn't** be found")
    ...
    async def valorant(self, ctx: Context, *, username: str): 
     ...
     data = await r.json()
     if data["status"] != 200: return await ctx.send( "Couldn't get valorant data of this player")
    ...
    async def clearsnipes(self, ctx: Context): 
        ...
        for l in lis:
            cursor.execute(f"DELETE FROM {l} WHERE guild_id = ?", (ctx.guild.id,))
        self.cursor.commit()
        cursor.close()
        return await ctx.send("Cleared all snipes from this server")
    ...
    async def editsnipe(self, ctx: Context, number: int=1): 
        ...
        self.cursor.execute("SELECT * FROM editsnipe WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, ctx.channel.id))
        results = self.cursor.fetchall()
        self.cursor.close()
        if len(results) == 0: 
            return await ctx.send("There are no edited messages in this channel")
        if number > len(results): 
            return await ctx.send(f"The maximum amount of snipes is **{len(results)}**")
    ...
    async def reactionsnipe(self, ctx: Context, number: int=1):
        self.cursor.execute("SELECT * FROM reactionsnipe WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, ctx.channel.id))
        results = self.cursor.fetchall()
        self.cursor.close()
        if len(results) == 0: 
            return await ctx.send("There are no reaction removed in this channel")
        if number > len(results): 
            return await ctx.send(f"The maximum amount of snipes is **{len(results)}**") 
        sniped = results[::-1][number-1]
    ...
    async def snipe(self, ctx: Context, *, number: int = 1):
        ...
        self.cursor.execute("SELECT * FROM snipe WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, ctx.channel.id))
        check = self.cursor.fetchall()
        if len(check) == 0:
            return await ctx.send("There are no deleted messages in this channel")
        if number > len(check):
            return await ctx.send(f"current snipe limit is **{len(check)}**".capitalize())
        try:
            sniped = check[::-1][number - 1]
        except IndexError:
            return await ctx.send("Invalid snipe number")
        em = Embed(color=self.bot.color, description=sniped[3])
        em.set_author(name=sniped[5], icon_url=sniped[6])
        em.set_footer(text="{}/{}".format(number, len(check)))
        if sniped[7] != "none":
            if ".mp4" in sniped[7] or ".mov" in sniped[7]:
                url = sniped[7]
    ...
    async def pastusernames(self, ctx, member: User = None):
       ...
       else: return await ctx.send( f"no logged usernames for **{member}**".capitalize())
    ...
    async def picperms(self, ctx: Context, member: Member, *, channel: TextChannel=None):
      ...
      if channel.permissions_for(member).attach_files and channel.permissions_for(member).embed_links:
       ...
       return await ctx.send(f"Removed pic perms from {member.mention} in {channel.mention}")       
      await channel.set_permissions(member, attach_files=True,embed_links=True)
      return await ctx.send(f"Added pic perms to {member.mention} in {channel.mention}")     
    ...
    async def youtubetomp3(self, ctx: Context, url: str): 
     async with ctx.typing():
      ...
      elif re['status'] == "failed": return await ctx.send("Failed to download the video as an mp3")
      else: rele = tuple([re['url'], re['filename']])   
      if rele is None: return await ctx.send("Failed to download the video as an mp3")
    ...
    async def twitter(self, ctx: Context, *, username: str): 
         ...
         if data["error"] is not None: return await ctx.send( f"[**@{username}**](https://twitter.com/{username}) doesn't exist")
    ...
    async def seen(self, ctx, *, member: Member):
        check = self.bot.db.execute("SELECT * FROM seen WHERE guild_id = {} AND user_id = {}".format(ctx.guild.id, member.id))
        if check is None: return await ctx.send( f"I didn't see **{member}**")
        ts = check['time']
    ...
    async def inrole(self, ctx: Context, *, role: Union[Role, str]):
            if isinstance(role, str): 
              role = ctx.find_role(role)
              if role is None: return await ctx.send( "This isn't a valid role")

            if len(role.members) == 0: return await ctx.send("Nobody (even u) has this role") 
            i=0
            k=1
            l=0
            mes = ""
    ...
    async def bans(self, ctx: Context): 
     ...
     if len(banned) == 0: return await ctx.send( "There are no banned people in this server")  
     i=0
     k=1
     l=0
     mes = ""
    ...
    async def boosters(self, ctx: Context):
            if not ctx.guild.premium_subscriber_role or len(ctx.guild.premium_subscriber_role.members) == 0: return await ctx.send( "this server doesn't have any boosters".capitalize())
            i=0
            k=1
            l=0
            mes = ""
    ...
    async def boosters_lost(self, ctx: Context): 
      ...
      if len(results) == 0: return await ctx.send( "There are no lost boosters")
      i=0
      k=1
      l=0
      mes = ""
    ...
    async def userinfo(self, ctx: Context, *, member: Union[Member, User]=None):
     ...
     if user.id in self.bot.owner_ids: 
       badges.append("<:bcrown:1104765614549123225>")
     if user.public_flags.active_developer: 
      badges.append("<:7011activedeveloperbadge:1104765491358212207>")
     ...
     if user.discriminator in discrim or user.display_avatar.is_animated() or user.banner is not None:
      badges.append("<:hotspotnitro:1104766353342206102>")
    ...
    async def banner(self, ctx: Context): 
        guild = ctx.guild
        if not guild.banner: return await ctx.send( "this server has no banner".capitalize())
    ...
    async def icon(self, ctx: Context, *, id: int=None): 
        guild = ctx.guild
        if not guild.icon: return await ctx.send( "this server has no icon".capitalize())
    ...
    async def splash(self, ctx: Context): 
        guild = ctx.guild
        if not guild.splash: return await ctx.send( "this server has no splash".capitalize())
    ...
    async def github(self, ctx, *, user: str):
      try:
        ...
        embed.set_thumbnail(url=avatar_url)
        await ctx.reply(embed=embed)
      except: return await ctx.send( f"Could not find [@{user}](https://github.com/{user})")
